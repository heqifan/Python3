# -*- coding: utf-8 -*-
"""
Created on Sat Dec 25 10:53:36 2021
@author: 树风
"""
import numpy as np
...
def NC_to_tiffs(data,out_path):
    ...
    print('Lon_分辨率',Lon_Res)
    print('Lat_分辨率',Lat_Res)
...
#folder_name = str(input("请输入您存放 NC 多时间序列数据的文件的路径———————:"))    这是exe程序中要用的
#nc_to_tif(Input_folder = folder_name,end_name='nc')   #用户需要输入 ：nc文件所放的文件夹的路径，默认输出至同一上级目录中
nc_to_tif(Input_folder = r'Z:\复旦wxx\pre\pre')            #用户需要输入 ：nc文件所放的文件夹的路径，默认输出至同一上级目录中
