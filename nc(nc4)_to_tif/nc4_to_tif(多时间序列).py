...
def nc_to_tif(Input_folder,end_name = 'nc4'):
...
'''输入路径不能有中文字符----------比如放在D盘中(目前我发现只有有多时间序列的nc文件才会有这个问题，，，)'''    
#folder_name = str(input("请输入您存放nc4数据的文件的路径———————:"))
#nc_to_tif(Input_folder = folder_name,end_name='nc4')   #用户需要输入 ：nc文件所放的文件夹的路径，默认输出至同级目录中，名为'out_...'
