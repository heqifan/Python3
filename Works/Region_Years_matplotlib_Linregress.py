...
for var in Vars:   #循环四个变量
    ...
    for rcp in RCPs:   #循环两种数据
        ...
        for num,value_data in enumerate([value_data1,value_data2,value_data3,value_data4,value_data5]):#循环每个分区的数据
            ...
            m_s_c_data = value_data[['Area_Value_'+ str(year) for year in range(styear,edyear+1)]].astype('float')  #获取每年的原始数据
            year_data =  value_data[['C_area']+['Area_Value_'+ str(year) for year in range(styear,edyear+1)]].astype('float')  #获取每年乘以对应面积的
            #计算每个分区的mean(),std(),cv()
            print('\n m_s_c_data中空值所在的行为：',m_s_c_data[m_s_c_data.isnull().T.any().T].shape)
            print('\n year_data中空值所在的行为：',year_data[year_data.isnull().T.any().T].shape)
            ...
            mean_data = np.nanmean(np.array(m_s_c_data),axis = 0)   #分区求平均
            std_data = np.nanstd(np.array(m_s_c_data),axis = 0)  #分区求标准差
