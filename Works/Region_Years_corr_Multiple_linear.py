...
def dm(data):
    ...
    '''计算每个分区每年的平均值'''
    data_mean = np.nansum(np.array(data.iloc[:,1:]),axis = 0)/np.nansum(data['C_area'])
...
for rcp in RCPs:   #循环两种数据类型
    ...
    prc_year_data,tas_year_data = pd.DataFrame(),pd.DataFrame()
    # npp_year_data['Lon_Lat'],prc_year_data['Lon_Lat'],tas_year_data['Lon_Lat'] = x_y_array['Lon_Lat'],x_y_array['Lon_Lat'],x_y_array['Lon_Lat']
    prc_year_data['Lon_Lat'],tas_year_data['Lon_Lat'] = x_y_array['Lon_Lat'],x_y_array['Lon_Lat']

    # npp_year_data['C_area'],prc_year_data['C_area'],tas_year_data['C_area'] = x_y_array['C_area'],x_y_array['C_area'],x_y_array['C_area']
    prc_year_data['C_area'],tas_year_data['C_area'] = x_y_array['C_area'],x_y_array['C_area']

    # for num,var in enumerate([npp_year_data,prc_year_data,tas_year_data]):   #循环三种变量
    for num,var in enumerate([prc_year_data,tas_year_data]):
        for year in range(styear,edyear+1):   #循环每一年的数据
            ...
            try:
                ...
                continue
            if Vars[num] == 'npp':
                var[year] = np.sum(data.iloc[:,69:105], axis=1)*var['C_area']   #npp求和
            elif Vars[num] == 'prc':
                var[year] = np.sum(data.iloc[:,2:], axis=1)*var['C_area']   #prc求和
            else:
                var[year] = np.average(data.iloc[:,2:], axis=1)*var['C_area']  #tas求平均
    print('所有年份数据已经存放完毕')   
    ...
    npp_join_data = pd.read_excel(r'D:\Cheng\全部数据0722.xlsx',sheet_name = 'npp' + rcp[-2:]).T
    columnNames = npp_join_data.iloc[0].apply(lambda x:round(x)) 
    npp_join_data = npp_join_data[1:] 
    npp_join_data.columns = columnNames
    npp_join_data.index = sort
    '''定义存放每个分区的数据'''
    ...
    for num,value_data in enumerate([value_data1,value_data2,value_data3,value_data4,value_data5]):#循环每个分区的数据
        ...
        for year in range(styear,eddyear+1):  #循环每年
            ...
            if num==4:  #如果循环到全国分区的话
                # npp = npp_join_data[npp_join_data['Region_value'].isin([1,2,3,4])].loc[:,['C_area']+all_yearss]
                npp = npp_join_data.iloc[[4]][all_yearss]
                prc = prc_join_data[prc_join_data['Region_value'].isin([1,2,3,4])].loc[:,['C_area']+all_yearss]
                tas = tas_join_data[tas_join_data['Region_value'].isin([1,2,3,4])].loc[:,['C_area']+all_yearss]
            else:  #如果还没循环到全国分区的话
                # npp = npp_join_data[npp_join_data['Region_value']==num+1].loc[:,['C_area']+all_yearss]
                npp = npp_join_data.iloc[[num]][all_yearss]
                prc = prc_join_data[prc_join_data['Region_value']==num+1].loc[:,['C_area']+all_yearss]
                tas = tas_join_data[tas_join_data['Region_value']==num+1].loc[:,['C_area']+all_yearss]            
            npp.dropna(axis=0, how='any', inplace=True)
            prc.dropna(axis=0, how='any', inplace=True)
            tas.dropna(axis=0, how='any', inplace=True)
            
            '''计算每10年的变异系数'''
            # print(f"{rcp}-{sort[num]}-npp的面积之和为: {npp['C_area']}")
            print(f"{rcp}-{sort[num]}-prc的面积之和为：{prc['C_area']}")
            print(f"{rcp}-{sort[num]}-tas的面积之和为：{tas['C_area']}")
            
            # npp_cv = (np.nanstd((np.nansum(np.array(npp.iloc[:,1:]),axis = 0)/np.nansum(npp['C_area']))))/(np.nanmean((np.nansum(np.array(npp.iloc[:,1:]),axis = 0)/np.nansum(npp['C_area']))))
            npp_cv = np.nanstd(npp)/np.nanmean(npp)
            prc_cv = (np.nanstd((np.nansum(np.array(prc.iloc[:,1:]),axis = 0)/np.nansum(prc['C_area']))))/(np.nanmean((np.nansum(np.array(prc.iloc[:,1:]),axis = 0)/np.nansum(prc['C_area']))))
            tas_cv = (np.nanstd((np.nansum(np.array(tas.iloc[:,1:]),axis = 0)/np.nansum(tas['C_area']))))/(np.nanmean((np.nansum(np.array(tas.iloc[:,1:]),axis = 0)/np.nansum(tas['C_area'])))) 
        ...
        if num==4:  #如果循环到全国分区的话
            # npp_1 = npp_join_data[npp_join_data['Region_value'].isin([1,2,3,4])].loc[:,['C_area']+npp_fore_years]
            # npp_2 = npp_join_data[npp_join_data['Region_value'].isin([1,2,3,4])].loc[:,['C_area']+npp_behead_years]
            npp_1 = npp_join_data.iloc[[4]][npp_fore_years]
            npp_2 = npp_join_data.iloc[[4]][npp_behead_years]
            prc_ = prc_join_data[prc_join_data['Region_value'].isin([1,2,3,4])].loc[:,['C_area']+all_years]
            tas_ = tas_join_data[tas_join_data['Region_value'].isin([1,2,3,4])].loc[:,['C_area']+all_years]
            
        else :  #如果还没循环到全国分区的话
            # npp_1 = npp_join_data[npp_join_data['Region_value']==num+1].loc[:,['C_area']+npp_fore_years]
            # npp_2 = npp_join_data[npp_join_data['Region_value']==num+1].loc[:,['C_area']+npp_behead_years]
            npp_1 = npp_join_data.iloc[[num]][npp_fore_years]
            npp_2 = npp_join_data.iloc[[num]][npp_behead_years]
            prc_ = prc_join_data[prc_join_data['Region_value']==num+1].loc[:,['C_area']+all_years]
            tas_ = tas_join_data[tas_join_data['Region_value']==num+1].loc[:,['C_area']+all_years]
            
        '''清洗数据-平均'''
        # npp_1_mean,npp_2_mean,prc_mean,tas_mean = dm(npp_1),dm(npp_2),dm(prc_),dm(tas_)
        npp_1_mean,npp_2_mean,prc_mean,tas_mean = npp_1,npp_2,dm(prc_),dm(tas_)
        npp_mean = np.append(npp_1_mean.iloc[:1,:].values.flatten()[:-1],npp_2_mean.iloc[:1,:].values.flatten())
        ...
        npp_dt = np.append(npp_1_dt.flatten()[:-1],npp_2_dt.flatten())
        all_dt_data['npp'+ sort[num]] = npp_dt
        all_dt_data['prc'+ sort[num]] = prc_dt
        all_dt_data['tas'+ sort[num]] = tas_dt 
        ...
        all_st_data['prc'+ sort[num]] = prc_
        all_st_data['tas'+ sort[num]] = tas_
